...
def to_device_compatible(host_tensor, device, memory_config):
    try:
...
def host_tiling_upload(host_tensor, dtype, device, memory_config):
    tiled_on_host = ttnn.from_torch(host_tensor, dtype=dtype, layout=ttnn.TILE_LAYOUT)
    return to_device_compatible(tiled_on_host, device, memory_config)
...
def make_host_inputs(m: int, k: int, n: int):
    # Keep host tensors in bfloat16 for stable generation and torch golden path.
    # TTNN dtype conversion happens in from_torch(..., dtype=...).
...
def run_host_tiling_path(
    ...
    for iter_idx in range(iterations):
        ...
        a_dev = host_tiling_upload(a_host, dtype, device, memory_config)
        b_dev = host_tiling_upload(b_host, dtype, device, memory_config)
        synchronize(device)
...
def run_device_tiling_path(
    ...
    for iter_idx in range(iterations):
        ...
        t0 = time.perf_counter()
        p0 = time.perf_counter()
        a_rm = ttnn.from_torch(a_host, dtype=dtype, layout=row_major, device=device, memory_config=memory_config)
        b_rm = ttnn.from_torch(b_host, dtype=dtype, layout=row_major, device=device, memory_config=memory_config)
        a_dev = ttnn.to_layout(a_rm, tile)
        b_dev = ttnn.to_layout(b_rm, tile)
        synchronize(device)
        p1 = time.perf_counter()
        ...
        o0 = time.perf_counter()
        out_rm = ttnn.to_layout(out_dev, row_major)
...
def parse_args():
    parser = argparse.ArgumentParser(description="TTNN matmul benchmark (new API): host vs device tiling paths")
