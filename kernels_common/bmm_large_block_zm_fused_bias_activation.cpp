...
void MAIN {
  ...
  uint32_t in0_block_num_tiles = get_compile_time_arg_val(
  ...
  uint32_t in1_num_subblocks = get_compile_time_arg_val(
      4); // outer column block size (in inner column blocks)
  uint32_t in1_block_num_tiles = get_compile_time_arg_val(
  ...
  uint32_t num_blocks_w_dim = get_compile_time_arg_val(
      12); // outer N-dim blocks (per_core_N / out_block_w)
  uint32_t num_blocks_h_dim = get_compile_time_arg_val(
      13); // outer M-dim blocks (per_core_M / out_block_h)
  ...
  init_bcast<EltwiseBinaryType::ELWADD, BroadcastType::ROW>(
      mm_bias_intermediate_cb_id, bias_cb_id, out_cb_id);
#endif
        ...
        for (uint32_t block = 0; block < num_blocks; block++) {
          ...
          for (uint32_t in0_subblock = 0; in0_subblock < in0_num_subblocks;
            ...
            for (uint32_t in1_subblock = 0; in1_subblock < in1_num_subblocks;
              ...
              if (enable_reload) {
                // Reconfigure input
                copy_tile_to_dst_init_short_with_dt(in1_cb_id,
                                                    mm_partials_cb_id);
              ...
              for (uint32_t h = 0; h < out_subblock_h; h++) {
                for (uint32_t w = 0; w < out_subblock_w; w++) {
                  ...
                  for (uint32_t inner_dim = 0; inner_dim < in0_block_w;
                       inner_dim++) {
                    int in0_index = in0_index_subblock_offset +
                                    in0_index_h_offset + inner_dim;
                    int in1_index = in1_index_subblock_offset +
                                    in1_index_inner_dim_offset + w;
                    matmul_tiles(in0_cb_id, in1_cb_id, in0_index, in1_index,
                                 dst_index, false);
              ...
              if (last_out) {
                ...
                cb_reserve_back(mm_bias_intermediate_cb_id,
                                out_subblock_num_tiles);
                for (uint32_t i = 0; i < out_subblock_num_tiles; i++) {
                  pack_tile(i, mm_bias_intermediate_cb_id);
                }
                cb_push_back(mm_bias_intermediate_cb_id,
                             out_subblock_num_tiles);
                release_dst();

                // Redundant wait since we know data was just pushed
                cb_wait_front(mm_bias_intermediate_cb_id,
                              out_subblock_num_tiles);
                cb_wait_front(bias_cb_id, in1_per_core_w);
                add_bcast_rows_init_short(mm_bias_intermediate_cb_id,
                                          bias_cb_id);
                ...
                for (uint32_t i = 0, j = 0; j < out_subblock_h; j++) {
                  ...
                  for (uint32_t k = 0; k < out_subblock_w; k++, i++) {
                    add_tiles_bcast_rows(mm_bias_intermediate_cb_id, bias_cb_id,
                                         i, bcast_tile_idx, i);
                    bcast_tile_idx++;
                  }
                }
                cb_pop_front(mm_bias_intermediate_cb_id,
                             out_subblock_num_tiles);
                ...
                // Wait for tiles in output buffer to be written out since
                // interm and output share memory
                if (block == 0) {
