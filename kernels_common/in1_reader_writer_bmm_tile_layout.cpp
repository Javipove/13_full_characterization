        ...
        if (w < last_block_w) {
          ...
          if (w == in1_identity_offset) {
            uint64_t l1_buffer_noc_addr =
                get_noc_addr(noc_x, noc_y, in1_tensor_addr);
            noc_async_read(l1_buffer_noc_addr, l1_write_addr_in1,
      ...
      for (uint32_t h = 0; h < out_subblock_h_; h++) {
        ...
        for (uint32_t w = 0; w < out_subblock_w_; w++) {
          uint32_t l1_buffer_addr =
              out_tensor_addr + (out_tensor_tile_id * single_tile_size_bytes);
          uint64_t l1_buffer_noc_addr =
              get_noc_addr(noc_x, noc_y, l1_buffer_addr);
          noc_async_write(l1_read_addr, l1_buffer_noc_addr,
                          single_tile_size_bytes);
